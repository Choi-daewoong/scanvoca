...
- **`deploy-final.ps1`은 `.env`에 없는 키를 빈 문자열로 덮어쓴다**: `--set-env-vars`는 지정한 값을 그대로 설정하므로, PowerShell이 `.env`에 없는 키를 `$null`→빈 문자열로 보간하면 운영 환경변수가 조용히 지워진다. 이렇게 GITHUB_REPO/GITHUB_BRANCH가 빈 값이 되어 블로그 게재가 계속 502를 내는 장애가 실제 발생했다(2026-07-15, 원인 발견까지 며칠 걸림). 새 env var를 스크립트에 추가할 때는 `.env`에도 반드시 같이 넣고, 애매하면 배포 후 `gcloud run services describe`로 실제 값 길이/내용을 확인한다.
