...
첨부된 PDF는 실제 수능/모의고사 영어 문제지입니다. 18번 이후 문항 번호만 빠짐없이 찾아, 각
문항의 유형을 아래 5가지 중 하나로 분류하세요. 본문 내용은 이 단계에서 추출하지 마세요 —
문항 번호와 유형 분류만 필요합니다.

1~17번(듣기 문항)은 절대 포함하지 마세요. 듣기 문항은 대화 음성이 문제지에 인쇄되지 않으므로
— 표나 도표가 함께 제시되는 문항(예: 스터디 룸 예약표를 보며 대화를 듣는 유형)도 마찬가지로
인용할 지문이 없습니다. "표를 보면서 대화를 듣고"처럼 듣기를 지시하는 문항은 유형이 도표처럼
보여도 절대 포함하지 마세요.
